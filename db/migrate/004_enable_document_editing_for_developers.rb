class EnableDocumentEditingForDevelopers < ActiveRecord::Migration[6.1]
  def up
    developer = Role.where('LOWER(name) = ?', 'developer').first
    return unless developer

    permissions = Array(developer.permissions).map(&:to_sym)
    developer.update!(permissions: permissions | [:edit_documents])
  end

  # Role permissions are administrator-owned configuration after installation.
  # Rolling back plugin schema must not silently revoke a permission which an
